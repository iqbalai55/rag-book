...
# ------------------ INGEST BOOK ENDPOINT (ASYNC, 202) ------------------
...
async def delete_book_chunks(
    ...
    worker sees the row has moved out of `running` on its next tick and
    skips writing chunks.
    """
    async with cache_manager.acquire_book_lock(book_id):
        # 0) Short-circuit any in-flight ingest for this book
        try:
